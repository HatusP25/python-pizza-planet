...
def __create_sizes_and_ingredients_beverages(ingredients: list, sizes: list, beverages: list):
...
def test_create(app, ingredients, size, client_data, beverages):
    created_size, created_ingredients, created_beverages = __create_sizes_and_ingredients_beverages(ingredients, [size],
                                                                                                    beverages)
...
def test_calculate_order_price(app, ingredients, size, client_data, beverages):
    ...
    pytest.assume(created_order['total_price'] == round(
        created_size['price'] + sum(ingredient['price'] for ingredient in created_ingredients) + sum(
            beverage['price'] for beverage in created_beverages), 2))
...
def test_get_by_id(app, ingredients, size, client_data, beverages):
    ...
    for param, value in created_order.items():
        ...
        beverages_in_order = set(item['beverage']['_id'] for item in created_order['beverage'])
        pytest.assume(not beverages_in_order.difference(beverages_ids))


def test_get_all(app, ingredients, sizes, client_data, beverages):
    created_sizes, created_ingredients, created_beverages = __create_sizes_and_ingredients_beverages(ingredients, sizes,
                                                                                                     beverages)
    created_orders = []
    for _ in range(5):
        order = __order(shuffle_list(created_ingredients)[:3], get_random_choice(created_sizes), client_data,
                        shuffle_list(created_beverages)[:2])
        created_order, _ = OrderController.create(order)
